...
SELECT code, name, population FROM countries WHERE continent = 'Europe' AND region = 'Southern Europe' ORDER BY population ASC LIMIT 1;
...
SELECT countrycode, language FROM countrylanguages WHERE language = 'Italian' AND countrycode NOT IN ( SELECT countrycode FROM countrylanguages WHERE language != 'Italian');
...
SELECT name, countrycode FROM cities WHERE name LIKE 'Serra%' AND countrycode IN ( SELECT code FROM countries WHERE continent = 'South America' );
...
SELECT name FROM cities WHERE id = (SELECT capital FROM countries WHERE code = 'BRA');
